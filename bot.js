...
const {TPAcceptTime, ErrorMessage, UnstockedItemMessage, ExpiredTP ,AcceptTPASK, UTPAcceptMSG, UTPCancelMSG, LoginMSG, RegisterMSG, username, password, host, port, version, TPDelay, HomeDelay, LavaCoords, BotOwner, HomeMessage, HomeCancelMSG, AfterRegisterMSG, AfterLoginMSG } = require('./config.json');
...
function initBot() {
  ...
  async function ObtenerUsuariosEnLinea() {//Funcion para acceder de forma externa 
    return bot.players
  }
  ...
  async function expulsar() {
    ...
    for (var i = 0; i < cantidaditems; i++) { //Por cada item que tenga en el inventario:
      ...
      if (bot.inventory.items()[itemindex] != undefined) { //Si el item con el indice actual existe en el inventario
  ...
  async function Suicidio() {
    ...
    bot.pathfinder.setGoal(new GoalNear(LavaCoords[0], LavaCoords[1], LavaCoords[2], 2)) //Se mueve a la lava para matar al bot
    ShouldBotBeKilled = false
  }

  async function QueueDelivery(items, usuario, RequestID){//Funcion accesada remotamente, agrega datos a una lista, contiene el usuario y un array de items
  ...
  function HandleDelivery(){
      if (Object.keys(QueueItems).length > 0 && isBotBussy == false){ //Si el bot no esta ocupado en otro envio y la cola de items es mayor que 0 envia el delivery y lo pone como envio actual
        SendDelivery(QueueItems[Object.keys(QueueItems)[0]].Items,QueueItems[Object.keys(QueueItems)[0]].Usuario, Object.keys(QueueItems)[0])
  ...
  async function SendDelivery(identificador, usuario, RequestID) {
    ...
    bot.once('death', () => { //Cuando el bot muera reestablece valores importantes para evitar bugs, el once hace que solo se ejecute una vez y no sea constante
      ShouldBotBeKilled = false
    ...
    for (var i = 0; i < identificador.length; i++) {
      const Skull = bot.findBlock({ //Skull es el bloque kit que buscamos, entonces se va a dirigir a el bloque
      ...
      const chest = bot.findBlock({//Encuentra un cofre que tiene que estar al lado del bloque representativo del kit
  ...
  bot.on('message', message => { // logs login
    ...
    if (message.toString() == HomeCancelMSG) { //Si la peticion de teletransporte se cancela
